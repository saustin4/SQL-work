...
select
    honors.student_name
     ,honors.match_id
    ,max(enter_term) as enter_term
    ,max(grad_term) as grad_term
    ,min(to_number(grad_term)-to_number(enter_term)) as tim 
    ,max(grad_date) as grad_date
    ,ttd --time to degree
    ...
    , subject 
from honors
left outer join local.wh_students
	on honors.match_id = wh_students.id	
left outer join local.wh_timing
	on wh_students.timing_id = wh_timing.timing_id
left outer join local.retention ret  
    on wh_students.id = ret.id
    and retention_type = 'FF'
left outer join local.ttd ttd 
    on wh_students.id = ttd.id
left outer join local.schev_ce_courses ce 
    on wh_students.id = ce.id
    and ce.course_number = '488'
left outer join local.schev_dc dc 
